...
namespace Vision.Difference {
    ...
    public interface IDiffer {
        ...
        /// <param name="ignoreWhiteSpace">如果设为<see cref="true"/>，在比较行是否相同时会忽略空格差异</param>
        /// <param name="ignoreCase">比较过程是否区分大小写</param>
        /// <param name="chunker">用于提取单词和单区段的组件</param>
    ...
    public class Differ : IDiffer {
        ...
        public DiffResult CreateDiffs(string oldText, string newText, bool ignoreWhiteSpace, bool ignoreCase, IChunker chunker) {
            ...
            var modOld = new Modification(oldText);
            var modNew = new Modification(newText);
        ...
        private static EditLengthResult CalculateEditLength(int[] A, int startA, int endA, int[] B, int startB, int endB, int[] forwardDiagonal, int[] reverseDiagonal) {
            ...
            throw new Exception("不可访问的代码");
        }

        protected static void BuildModificationData(Modification A, Modification B) {
        ...
        private static void BuildPieceHashes(IDictionary<string, int> pieceHash, Modification data, bool ignoreWhitespace, bool ignoreCase, IChunker chunker) {
